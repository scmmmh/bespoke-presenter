...
module.exports = function(options) {
  ...
  return function(deck) {
    ...
    function cloneHeader(source, target) {
        ...
        var sourceClone = source.document.querySelectorAll('head title, head link, head meta');
        for(var idx = 0; idx < sourceClone.length; idx++) {
            targetHead.appendChild(sourceClone[idx].cloneNode(true));
        }
    }

    function constructSlide() {
        var slave = window.open('#', 'bespoke-presenter-slave');
        cloneHeader(window, slave);
